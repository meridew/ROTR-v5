...
func init_mob_pool_inactive():
	...
	for i in range(mobs_max):
		mob = BaseMob.new("mob",10,100,base_speed,50,base_fps,base_acceleration,100,1,0.2).mob
		disable(mob)
...
func init_boss_mob_pool_inactive():
	...
	for i in range(boss_mobs_max):
		boss_mob = BaseMob.new("boss_mob",50,1000,base_speed,5000,base_fps,base_acceleration,1000,2,0.2).mob
		disable(boss_mob)
